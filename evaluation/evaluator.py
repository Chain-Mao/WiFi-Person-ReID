...
def inference_on_dataset(model, data_loader, evaluator, flip_test=False):
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            extracted_features.append(outputs.detach().cpu().numpy())
            extracted_targets.append(inputs["targets"].detach().cpu().numpy())
    ...
    extracted_targets = np.concatenate(extracted_targets, axis=0)
    np.save("/data1/fast-reid-wifi/tools/output/extracted_features.npy", extracted_features)
    np.save("/data1/fast-reid-wifi/tools/output/extracted_targets.npy", extracted_targets)
